...
**npm workspaces(2026-08-31 迁移)**:仓库根 `package.json` 声明 `workspaces: ["apps/*", "packages/*"]`,
...
**CI(GitHub Actions,`.github/workflows/ci.yml`,2026-08-31 起)**:push / PR 到 main 自动跑两个 job——
`checks`(npm ci → prisma generate → `check:all` → `test:mock`)与 `e2e`(service 容器起真
Postgres/Redis → 迁移 + 三段 seed → 全量 e2e,`E2E_LLM_ISOLATION=1`)。CI 只调用根编排脚本,
...
lab(build)→ server(build);另有 `npm run test:mock`(三端 mock 冒烟)与 `npm run e2e`(后端 e2e,
需 dev 库与 seed)。单项脚本 `check:contracts / check:ui / check:admin / check:teacher /
check:student / check:lab / check:server` 可按需单跑。

| 范围 | 命令 | 位置 |
|---|---|---|
| 后端 e2e(真库真 Redis) | 根 `npm run e2e`(即 `npm -w @qiming/server run test`) | `test/*.e2e-spec.ts` + `test/fixtures/` |
| 前端单测 | `npm -w @qiming/<端> run test`(vitest;lab 无测试) | `src/**/__tests__/` |
...
9. ~~没有 monorepo 工具~~ **已解决(2026-08-31)**:npm workspaces + 根统一 lockfile + 根编排脚本
   ...
   统一 lockfile(首例:`task/lab-anim-motion` 合并时会遇到 `apps/lab/package-lock.json` 的
   修改/删除冲突,照此处理)。
...
已反映 npm workspaces 迁移)。重大改动后需更新——更新触发条件见 `docs/README.md` 的「维护要求」。*

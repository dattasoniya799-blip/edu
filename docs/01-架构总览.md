...
>
> 本文以**当前实际代码**为准。若与已归档的早期设计文档、任务卡(`_archive/启明智学-开工包-2026-06/`)冲突,以代码为准;
...
学生端做作业、错题本、实时课堂与 AI 答疑;AI 能力统一经后端一个 LLM 网关接入,可按功能在
真实供应商与 mock 之间切换。**2026-08-31 起仅 AI 答疑(qa)在线**,其余 AI 呈现(预批/伴学旁白/
学情诊断/生成课件)因实现为 stub/模板已整体下线,重启条件见 `docs/需求文档/2026-08-31-下线功能需求留档.md`。
...
| 前端 | React 18 + TypeScript 5 + Vite 5 + react-router 6 | `apps/{admin,teacher,student}`、`labs/playground` |
...
**npm workspaces(2026-08-31 迁移)**:仓库根 `package.json` 声明 `workspaces: ["apps/*", "packages/*", "labs/*"]`,
全仓**单一 `package-lock.json`**,依赖统一提升到根 `node_modules`(版本冲突的包自动嵌套,如
`packages/ui` 的 vitest 2)。在仓库根跑一次 `npm install` 即装齐所有端;根脚本 `check:all` 一键跑
全量门禁(见第九节)。跨包引用仍靠 **TS path 映射 + Vite alias 直接指向源码**(不经过构建产物),见第四节。
...
├── labs/             实验区(永不部署,不进主线门禁;2026-09-02 自 apps/lab 迁出)
│   └── playground/   前端实验公共外壳:动画课堂、知识点动画
├── packages/
│   ├── contracts/    API/WS/设计令牌契约 —— 全仓「唯一事实」,受协作纪律保护
│   │   └── CHANGELOG.md   契约变更记录(改契约的同一 PR 追加)
...
├── docs/             ← 本目录(00 协作纪律 / 01 架构总览 / 02 前端风格基线 / …)
├── AGENTS.md         AI 会话入口:先读什么、红线是什么(CLAUDE.md 转指向它)
├── CHANGELOG.md      交付记录/变更日志(按任务波次追加,不是架构说明)
└── README.md         仓库入口
```

### 各 app / package / lab 一览

| 目录 | 包名 | 职责 | 开发端口 | 启动 |
|---|---|---|---|---|
| `apps/server` | `@qiming/server` | 全部业务 API(`/api/v1`)、WS 课堂网关、队列 worker、AI 网关 | 3000 | `npm run start:dev` |
| `apps/admin` | `@qiming/admin` | 机构管理员:师生/课程/名单、实验室(讲解件展台 + 实验服务入口:知识图谱 / 讲题白板)、AI 用量与接口配置、实验室管理、平台设置 | 5173 | `npm run dev` |
...
| `labs/playground` | `@qiming/lab` | 前端实验区(动画课堂、知识点动画);**不进 deploy、不进 check:all** | 5176 | `npm run dev` |
| `labs/knowledge-graph` | `@qiming-labs/knowledge-graph` | 初中数理化知识图谱**动态**服务 + 浏览器(2026-09-17):启动时读 `data/knowledge-graphs/v2` 实时构图,`fs.watch` 热重建,零 npm 依赖;管理端「实验室 → 实验服务」链过来。正式形态(进 DB + `/kp/*` 接口)另立项 | 8787 | `node server.mjs` |
| `labs/lecture-board` | `@qiming-labs/lecture-board` | 讲题白板原型(2026-09-17):上传题目截图 + 答案 → Qwen 识题/规划/剧本 → Excalidraw 白板边讲边写(KaTeX、Seedream 配图、模板与 HTML 动画、CosyVoice);`server/`(Fastify + SSE)与 `web/`(Vite React)各自 `npm install`;方案见工作区 `_lab/讲题PPT/讲题白板-整体方案-2026-09-17.md` | 4310 / 4311 | `npm run dev` |
...
```bash
npm install                                              # 仓库根,一次装齐全部 workspace
...
- `packages/ui` 的运行时依赖(katex、qrcode)与所有 workspace 依赖一样装在**仓库根 `node_modules`**
  (2026-08-31 起 npm workspaces 统一安装);Docker 构建按根 lockfile 装依赖,无需单独处理 ui。
- `packages/contracts` 声明了 `"type": "module"`(2026-08-31):tsx/node 直跑的脚本(如三端
  `test:mock`)经 node_modules 符号链接解析 contracts 源码时,需要它才能拿到具名导出;
  vite/tsc/nest build 路径不受影响。
- 三端 `vitest.config.ts` 曾把 react/katex 硬别名到 `<app>/node_modules`(旧布局的双 React 修复),
  workspaces 提升后这些路径不存在,已删除——**不要再往 vitest 配置里加指向 app 本地 node_modules 的别名**。
- Vite 的 `server.fs.allow` 在 teacher 与 labs/playground 里显式放开了仓库根,否则 dev server 读不到 `packages/`。
...
- 密码 argon2(`auth/password.util.ts`);重置 / 改密时往 Redis 写吊销时间戳 `auth:pwdreset:{uid}`,
  `JwtAuthGuard` 比对 `iat` 使旧 token 立即失效;**停用账号**写 `auth:disabled:{uid}`,已签发 token 立即 401「账号已停用」,
  启用时删位并写吊销 epoch(启用前旧 token 不复活)。HTTP 守卫与课堂 WS 握手共用 `auth/pwd-reset.ts#checkTokenRevoked`
  (2026-09-02 走查 D-1)。教师登录取同号候选逐一验密(同号不再锁死),停用者密码正确时 403「账号已停用」而非「账号或密码错误」。
- 学生学号登录 trim + 大写归一(管理端自定义学号落库亦大写);学习时段门禁按机构时区判定
  (`common/student-hours.ts`,`ORG_TIMEZONE` 缺省 Asia/Shanghai;镜像已设 `TZ`),与服务器进程时区无关。
...
| 一次性凭证(上传 token、课件任务态 24h TTL) | 各模块自管 | 无 |
| access token 吊销位(改密 / 停用,TTL = JWT_ACCESS_TTL) | `auth:pwdreset:{uid}`、`auth:disabled:{uid}` | 无(e2e globalSetup 会清 `auth:*`:整库重建后 uid 复用会被上一轮残留误杀) |
...
业务模块(qa / pre-grading / companion / diagnosis / courseware)
        │  只依赖 LlmGatewayService 接口,不 import 任何供应商 SDK(协作纪律第 4 条)
        ▼
LlmGatewayService  ── 额度预检(Redis 月成本 × ai_quotas.over_policy)
 (ai/llm/)         ── RouteTableService.resolve(feature) → provider/model/fallback
        ...
        ▼
providers/  mock · openai_compatible(文本)   |   mock_image · openai_compatible_image · ark_image(生图)
```
...
- **路由表三层**:`ai/config/ai-routes.default.json`(默认,全 mock)→ env 感知
  (配了 `LLM_API_KEY` 则**只有 qa** 默认走真实;预批 / 伴学 / 诊断 / 课件已下线,配了 key 也保持 mock,重启时由管理端显式切;
  2026-09-02 走查 C-3 之前是四功能随 key 全 real)→ Redis `a7:ai:routes` 覆盖(管理端 `PUT /admin/ai/routes` 逐功能切真假,不重启生效;
  管理端页面对已下线四项只登记「已下线」不给开关)。
...
- **文本与生图是两套独立 env**(`LLM_*` / `IMAGE_*`),各自的 key 只在 provider 内部读取,不进日志。
  生图真实供应商两家(2026-09-17 起):`openai_compatible_image`(OpenAI images 形状,回 PNG)与 `ark_image`
  (火山方舟 Seedream,`response_format` 强制 b64、`size` 用档位、回 JPEG),共用同组 `IMAGE_*`,由 `IMAGE_PROVIDER` 选一家作为
  `courseware=real` 的条目与管理端「测试连接」的目标;课件落盘扩展名按字节魔数取 `.png`/`.jpg`(`courseware-page.service.ts`)。
- **批改域的接缝**:`grading` 不直接依赖 `ai`,而是注入 `AI_GATEWAY` token(定义在 `grading/ai/ai-gateway.ts`),
  由 `grading.module.ts` 绑到 `AiModule` 导出的 `LlmPreGradeGateway`。
  (历史上同目录的规则版 `stub-ai-gateway.ts` 已于 2026-08-31 删除,等价 mock 规则在 `ai/llm/providers/mock.provider.ts`。)
...
- **文件存储**:`upload/storage/` 有 local 与 oss 两个适配器,`STORAGE_DRIVER` 切换;
  ...
  `GET /uploads/view-url?ossKey=`(带 token)拿签名直链 → `@Public GET /storage/*`(按扩展名给 Content-Type + nosniff,
  html/svg 等可执行类型刻意回退 octet-stream 走下载;2026-09-02 走查 G-2 之前恒 octet-stream,预览变下载)。
...
当前两项**均为 off**(2026-08-31 假功能全部下线):`ai_courseware`(teacher)、`photo_pregrade`(student)。
下线全景与重启条件见 `docs/需求文档/2026-08-31-下线功能需求留档.md`——除这两项外,同波还下线了
课堂伴学旁白(narration,服务端不再下发)、AI 学情诊断(org 设置默认关 + 教师端按钮隐藏)、
学生课堂小结的「AI」包装(改中性文案)。
...
admin / teacher / student 三端结构高度一致,看懂一个就看懂其余(labs/playground 是简化版:没有 `api.ts`、`auth/`、`mocks/`):
...
| `/lab` | 实验室(讲解件展台,静态 HTML 在 `public/lab/`) |
...
| `/papers`、`/papers/new`、`/papers/:id/edit` | 试卷库(学科筛选 / 草稿「发布」;编辑页「保存草稿 / 创建并发布」;契约 `PaperDto.subject/kpNodes` 为服务端聚合只读) |
| `/bank`、`/bank/new`、`/bank/:id/edit` | 题库维护与录题(含公式键盘、插图上传) |
| `/knowledge`、`/resources`、`/analytics` | 知识点内容库(学科切换,默认教师在带课程学科)、资源库(上传后引导归档知识点,`PUT /resources/{id}.kpNodeId`)、学情分析 |
...
**labs/playground(5176)** — 没有路由表,`src/experiments/registry.ts` 是登记表,首页按表列清单。
两个实验:`dynamic-lecture/`(动画课堂:zod 剧本契约 + 坐标系 Canvas 缓动播放器 + 五份剧本 +
`tools/check-scripts.ts` 质检门)与 `kp-animations/`(单文件零依赖交互 HTML 素材车间,
`public/animations/` + `tools/validate-animations.mjs` 校验器)。
**永不部署**:不在 `deploy/web.Dockerfile` 里;**不进 `check:all`**,由 CI 的非阻塞 `labs` job 跑 `check:labs`。
细节见 `labs/playground/README.md`。
...
`packages/contracts` 受协作纪律(`docs/00`)保护:**改它需要先提「契约变更申请」并经人工批准**,
...
2026-09-02 走查后的两条口径(用户拍板 F-1 / F-5):
- **客观错题交卷即入账**:卷面含需复核题时,`AttemptService.submit` 立即调 `GradingService.accountObjectiveOnSubmit`
  只入账客观题;之后 `settleAttempt` 对同一 attempt 只入账需复核题(课堂结算 `settleInClassAttempts` 同口径),二者互斥不重复计数。
  纯客观卷仍在 `finalizeAttempt` 一次性入账。
- **出分自动生成订正作业**:`settleAttempt` 后对 `kind=homework` 且有讲次锚点的作业,按该生本次答错的客观题建 `订正 · 原卷名`
  practice 卷 + `kind=correction` 作业(target=本人,不计分,teacherId 沿用来源,每(讲次,学生)幂等一份;
  `WrongBookService.createCorrectionForAttempt`);学生端时间线据 pending 的 correction 作业显示「订正错题」。
- 错题本 `errorTags` 为空(预批已下线)时回退为题目的教材知识点名,分组仍有意义。
...
课堂两处 2026-09-02 补齐:
- **课件下发**:`ClassroomService.buildCourseware` 对讲解环节——有结构化 `config.pages` / `meta.pages` 按逐页下发;否则把挂的普通资源
  以一页 `CoursewarePageView.resource{type,name,url}`(签名直链,同 view-url 机制,重连新快照重签;ossKey 须属本机构)下发,
  学生端 `LectureSegment` 按 type 渲染(image / pdf / video / interactive 沙箱 iframe / ppt 下载)。此前非结构化课件在课堂里永远「暂不可用」。
- **教师控场**:监控页 `pages/monitor/source.ts` 的 `MonitorSource.control` 发 `class:control`(pause / resume / force_segment / end),
  `class:join` 的 ack 快照给会话状态与环节表(也据此判定「已连上」,仅作业环节的会话不再骨架屏);此前监控页只监听、课堂在 UI 上永远结束不了。
...
- 两个 Dockerfile 的**构建上下文都必须是仓库根**(因为要拷 `packages/`),装依赖层按 npm workspaces
  组织(2026-08-31):先 COPY 根 lockfile + **全部 7 个** workspace 的 `package.json`(npm ci 校验整片
  森林,缺一报错),再 `npm ci --workspace=<目标>` 只装目标子树。
  `apps/server/Dockerfile`:build → proddeps(`--omit=dev`,另 `--no-save` 补装 prisma CLI/tsx)→
  runtime(拷根 `node_modules` 整层,非 root,`/healthz` HEALTHCHECK)。
  `deploy/web.Dockerfile`:deps 阶段一次装齐三端 + ui(含 ui 的 @types devDeps,tsc 编译 ui 源码要用),
  三端构建阶段共享 deps 层,产物合入一个 nginx 镜像。
  **取舍**:统一 lockfile 后,任一端改依赖都会使装依赖层缓存失效(旧「改单端不重装其它端」粒度不再有)。
...
**CI(GitHub Actions,`.github/workflows/ci.yml`,2026-08-31 起)**:push / PR 到 main 自动跑三个 job——
`checks`(npm ci → prisma generate → `check:all` → `test:mock`)、`e2e`(service 容器起真
Postgres/Redis → 迁移 + 三段 seed → 全量 e2e,`E2E_LLM_ISOLATION=1`),以及 **非阻塞**的 `labs`
(`check:labs`,`continue-on-error`,红了只提示不拦合并;2026-09-02 起)。CI 只调用根编排脚本,
本地绿即 CI 绿;Docker 镜像构建不进默认 CI(分钟数考虑),改动 Dockerfile 时本机构建验证。

**根 `package.json` 的编排脚本(2026-08-31 起,本地与 CI 同源)**:在仓库根跑
`npm run check:all` = contracts(check+lint:openapi)→ ui(test+check)→ 三端(build+test)→
server(build);另有 `npm run test:mock`(三端 mock 冒烟)、`npm run e2e`(后端 e2e,需 dev 库与
seed)与 `npm run check:labs`(实验区:剧本质检 + 动画校验 + build,**不在 check:all 内**)。
单项脚本 `check:contracts / check:ui / check:admin / check:teacher / check:student / check:server`
可按需单跑。

| 范围 | 命令 | 位置 |
|---|---|---|
| 后端 e2e(真库真 Redis) | 根 `npm run e2e`(即 `npm -w @qiming/server run test`) | `test/*.e2e-spec.ts` + `test/fixtures/` |
| 前端单测 | `npm -w @qiming/<端> run test`(vitest;labs/playground 无测试) | `src/**/__tests__/` |
| 前端 mock 冒烟 | 根 `npm run test:mock`(三端串行) | `scripts/mock-smoke.mts` |
| 类型门禁 | 各端 `npm run build`(先 `tsc --noEmit`)、`packages/*` 的 `npm run check` | — |
| 契约校验 | `npm -w @qiming/contracts run lint:openapi` | swagger-parser |
...
4. `apps/server/test/` 加 e2e,**必须含跨租户 404 用例**((`docs/00` 第 7 条))。
5. 前端调用侧:`api.get('/新路径')` 即可,类型自动来;mock 模式下还要在对应端
   `src/mocks/handlers.ts` 加 handler,否则 mock 下该页会空。

⚠️ **契约是协作纪律保护对象**(`docs/00` 第 8 条):改 `packages/contracts` 需要先提「契约变更申请」经人工批准,
并在同一提交把变更追加到 `packages/contracts/CHANGELOG.md`。
...
3. **然后必须回归三端**:`admin`、`teacher`、`student` 各跑 `npm run build`(含 `tsc --noEmit`)与
   `npm run test`(根 `check:all` 一把全跑)。`labs/playground` 同样直接编译 ui 源码,但不在门禁内——
   顺手跑一次 `npm run check:labs`,红了修掉即可,不阻塞合并。因为各端是**直接编译 ui 源码**的,没有版本缓冲。
4. 改 props 形状时优先**放宽**(如 `resolveSrc` 从同步拓宽为可同步可异步),避免同时改四端调用点。
5. 需要新的运行时依赖:在仓库根执行 `npm install <pkg> -w @qiming/ui`(写进 ui 的 `package.json`,
   统一 lockfile 随之更新);Docker 的 `deploy/web.Dockerfile` deps 阶段按根 lockfile 安装,自动覆盖。
...
3. **有三个真实依赖的端点没进 openapi**:`GET /uploads/view-url`、`PUT /uploads/local/:token`、
   `@Public GET /storage/*`。前端为此在 `api.ts` 里做了类型放宽和一处原生 fetch。协作纪律曾把它记为待收口红线。
4. **多租户安全缺口(单机构暂不可触发)**:`users.phone` 无唯一约束、登录不带机构标识,
   第二家机构上线前必须先做——改造方案已写好待命,见 `docs/03-多机构登录改造方案.md`。
   (paper/resource 的行级 owner 写校验经 2026-08-31 核实**已存在**:paper.update /
   resource.update+remove 均校验创建者本人或 admin,sec-back e2e #7 已固化,此前文档记录滞后。)
5. **OCR 是 stub**(`ai/ocr/ocr.service.ts` 的 `LocalOcrStub`),拍照预批因此被功能开关置 `off`。
   改预批逻辑的正确文件是 `ai/features/pre-grading.gateway.ts` 的 `LlmPreGradeGateway`
   (`AI_GATEWAY` token 的绑定见 `grading/grading.module.ts`);曾经易改错的历史实现
   `grading/ai/stub-ai-gateway.ts` 已于 2026-08-31 删除。
...
8. ~~没有 CI~~ **已解决(2026-08-31)**:GitHub Actions 双 job(checks + e2e)在 push/PR main 时
   强制全量门禁(见第九章)。注意 Docker 镜像构建不在默认 CI 内,改 Dockerfile 仍需本机验证。
9. ~~没有 monorepo 工具~~ **已解决(2026-08-31)**:npm workspaces + 根统一 lockfile + 根编排脚本
   `check:all`。遗留注意:给某端加依赖要用 `npm install <pkg> -w @qiming/<端>`(在仓库根执行);
   若有旧分支改过已删除的各目录 `package-lock.json`,合并时保留删除并在根重跑 `npm install` 刷新
   统一 lockfile(首例 `task/lab-anim-motion` 于 2026-09-02 合并时即如此处理:保留删除 →
   根 `npm install` → 删掉旧布局残留的各目录 `node_modules`(不删会遮蔽根依赖)→ `prisma generate`
   → `check:all`)。
...
12. `labs/playground` 永不部署,别把它加进 `deploy/web.Dockerfile`(两个 Dockerfile 只 COPY 它的
    `package.json` 供 npm ci 校验 workspace 森林);它也是唯一用 zod 的前端(与三端技术选型不同)。
    它不在 `check:all` 里——改 `packages/ui` 时若想顺手确认没把它弄红,单独跑 `npm run check:labs`。

13. **服务端比率字段一律 0–1**(`attendanceRate` / `homeworkRate` / `weekAttendanceRate` / 学生 `correctRate`),前端显示必须乘 100
    (admin `lib/format.ts#formatRate`、student `lib/format.ts#formatCorrectRate`);mock 数据也要写比值——mock 写 94.2 曾把「1%」的真实 bug 盖了一年。
14. **走查用的 Playwright 脚本在仓库外** `_lab/ui-walkthrough/`(`W1–W4/*.mjs`),修完前端想复查就重跑;它们依赖真实后端 + 三端 `VITE_USE_MOCK=false`。
---

*生成日期:2026-08-31(基于 `4e35b79` 撰写;同日随「结构性债务修改计划」各波次持续更新,
已反映 npm workspaces 迁移)。2026-09-02 更新:`apps/lab` 迁至 `labs/playground` 并退出 `check:all`、
CI 增非阻塞 `labs` job、协作纪律与契约变更记录进仓库(`docs/00`、`packages/contracts/CHANGELOG.md`);同日全面走查修复波次
(停用即时吊销 / 时区 / 错题即时入账 / 订正作业 / 试卷分类与草稿 / 课件下发 / 教师控场 / 比率格式化,见
`docs/需求文档/2026-09-02-系统全面走查-缺陷清单.md`)。
重大改动后需更新——更新触发条件见 `docs/README.md` 的「维护要求」。*

...
export const meetOnRoad: LectureScriptInput = {
  ...
  steps: [
    ...
    },
    {
      ...
      quiz: {
        question: '若乙车也改成从甲地出发(同向),还能在图上用交点表示「追上」吗?',
        options: ['能,两条都往右上,交点就是追上', '不能,同向要用距离差,对轴的方式不同', '能,只要速度不同就一定有交点'],
        answerIndex: 0,
        reveal:
          '同向追及时,慢车 y=40x、快车若晚出发才是另一条;若同时同地出发,两条线是过原点的射线,不相交。本题是相向,所以一条上一条下。选「能」时要想清楚出发地是否相同。本题正确答案按相向模型:交点就是相遇。',
      },
    },
  ],
  reference: [

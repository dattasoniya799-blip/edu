 ...
 *   PreGradingQueueService 分别由 AssignmentModule、GradingModule 导出;E1 起还依赖
 *   FeaturesModule 的 FeatureGateService —— 拍照预批入队门禁),禁止重写口径;
 *   A5 的 AttemptModule 未导出该服务,故在本模块按同一类再提供一份(无状态,零改动 A5 代码)
 * - ended 结算的课后作业发布调 A4 AssignmentService.create(禁止重写发布逻辑)
 */
@Module({
  imports: [AssignmentModule, GradingModule, AiModule, FeaturesModule],

...
> 本文件是历史交付/修复记录,按时间序追加,回答「为什么变成这样」。
> 想知道「现在长什么样」看 `docs/01-架构总览.md`;项目当前状态与路线图在仓库外的
> `../项目状态与路线图.md`。本文件原为仓库 README,2026-08-31 迁移至此,内容原样保留。
...
| 09-17 | `task/ark-image` | **生图接入火山方舟 Seedream**:新增 `ark_image` 供应商(`ai/llm/providers/ark-image.provider.ts`,`response_format` 强制 b64、`size` 档位、`watermark` 缺省关、回 JPEG);env `IMAGE_PROVIDER` 选真实生图供应商(`imageRealProvider`,缺省仍 `openai_compatible_image`),路由 real 条目 / 管理端「测试连接」按它分流;课件落盘扩展名按魔数 `.png/.jpg`;pricing 补 `doubao-seedream-5-0-260128`(估算);aiadmin e2e +3 例;编译产物真机冒烟 1 张 2K 通过。不改契约、不改 schema;`ai_courseware` 仍 off |
| 09-17 | `task/lecture-polish` | **讲题白板三轮打磨**:① 审题改为老师式分析(读懂题 → 考点定位 → 抓 2–3 处关键条件 → 求什么/怎么串,数据不逐条念;识题 ideas[0] 固定考点;校验器识别「逐条念条件」反模式);② 首页改鲸云AI品牌(Hero / 上传区 / 7 道示例题一键试讲 `GET /api/samples`+`POST /api/lessons/from-sample/:dir` / 已生成课程库);③ 运行问题复查:HTML 动画按容器自适应、动画卡去动作名、朗读成语白名单,修 4 个真 bug(重启半成品课清理、from-sample 路径穿越、上传魔数校验、SSE 终态关闭)。server 219 测 / web 124 测 |
| 09-17 | `task/walk-4` | **教师端走查修复**:试卷库点开崩溃(`PaperDto.questions/kpNodes` 直读 `.length` 零防守 → `paperQuestions()/paperKpNodes()` 统一兜底,四处替换 + `QuestionDto.*` 同类清扫,msw 坏记录回归 + e2e 数组不变式断言);挂载课件弹窗按「学科›年级›章节」+ 类型筛选、选随堂练/课后作业弹窗按「学科›类型」+ 年级推断分组(共用 `GroupedPicker`,全部用现有 `/kp/*` 字段拼目录,不改契约;+41 测)。课程「年级」字段的契约变更申请已写好待批 |
| 09-17 | `task/lab-kg-lecture` | **实验区两项进仓**:`labs/knowledge-graph`(初中数理化知识图谱动态服务 + 浏览器,读 `data/knowledge-graphs/v2` 实时构图、热重建,与 Python 导出对拍一致,18 测)+ `data/knowledge-graphs/v2` 三张图谱数据入库;`labs/lecture-board`(讲题白板原型:Qwen 识题/规划/剧本 → Excalidraw 白板讲题,server 149 测 / web 86 测,七道题零干预跑通);管理员实验室新增「实验服务」卡片(地址 `VITE_LAB_*` 可覆盖);`labs/*` 两个新 workspace 进根 lockfile。不改契约、不改 schema |
